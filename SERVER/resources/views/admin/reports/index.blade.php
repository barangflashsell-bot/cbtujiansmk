...
@section('title', 'Laporan & Analisis Akademik — CBT Administrator')
@section('page-title', 'Laporan & Analisis Akademik')
...
<div style="display: flex; flex-direction: column; gap: 24px;">
    ...
    <div>
        ...
        <p style="font-size: 0.85rem; color: var(--text-secondary);">
            Ringkasan statistik performa ujian, rekapitulasi nilai per rombel/kelas, dan analisis butir soal (Item Difficulty Analysis).
        </p>
    </div>
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
            ...
            <form method="GET" action="{{ route('admin.reports.index') }}" style="display: flex; gap: 8px; flex-wrap: wrap;">
                <input 
                    type="text" 
                    name="search" 
                    class="form-control" 
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div style="font-size: 0.8rem; color: var(--text-secondary);">
                                        Mapel: {{ $exam->subject->name ?? '-' }} | Pembuat: {{ $exam->creator->name ?? 'Admin' }}
                                    </div>
                                </td>
                                <td>
                                ...
                                <td style="text-align: right;">
                                    <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                        <a href="{{ route('admin.reports.exam', $exam->id) }}" class="btn btn-sm btn-primary" title="Lihat Rekapitulasi Statistik & Nilai Siswa">
                                            📊 Statistik Nilai
                                        </a>
                                        <a href="{{ route('admin.reports.item-analysis', $exam->id) }}" class="btn btn-sm btn-secondary" title="Analisis Tingkat Kesukaran Soal">
                                            🔍 Analisis Soal
                                        </a>
                                    </div>
                                </td>
        ...
        @else
            <div class="empty-state">
                ...
                <div class="empty-title">Belum ada paket ujian ditemukan</div>
                <div class="empty-desc">Pastikan jadwal paket ujian sudah dibuat pada modul Ujian.</div>
            </div>
        @endif
    </div>
    ...
    <div class="card">
        ...
        <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 16px;">
            Pilih rombel/kelas di bawah ini untuk melihat lembar rekapitulasi nilai seluruh siswa:
        </p>

        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px;">
            @forelse($classes as $c)
                <a href="{{ route('admin.reports.class', $c->id) }}" style="text-decoration: none; color: inherit; background: var(--bg-main); border: 1px solid var(--border-color); border-radius: var(--radius-sm); padding: 14px 16px; display: flex; align-items: center; justify-content: space-between; transition: border-color 0.15s ease, transform 0.15s ease;" onmouseover="this.style.borderColor='var(--primary)'; this.style.transform='translateY(-2px)';" onmouseout="this.style.borderColor='var(--border-color)'; this.style.transform='none';">
                    <div>
