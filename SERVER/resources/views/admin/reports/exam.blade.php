...
@section('title', 'Laporan Statistik Ujian: ' . $exam->title . ' — CBT Administrator')
@section('page-title', 'Laporan Statistik Ujian')
...
<div style="display: flex; flex-direction: column; gap: 24px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
        <div>
            ...
            <div style="font-size: 0.85rem; color: var(--text-secondary); display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
                ...
                <span>Pembuat: <strong>{{ $exam->creator->name ?? 'Admin' }}</strong></span>
            </div>
        </div>
        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
            <a href="{{ route('admin.reports.exam.export_csv', $exam->id) }}" class="btn btn-primary btn-sm">
                📥 Unduh Excel (CSV)
            </a>
            <a href="{{ route('admin.reports.exam.export_pdf', $exam->id) }}" target="_blank" class="btn btn-secondary btn-sm">
                🖨️ Cetak / PDF
            </a>
            <a href="{{ route('admin.reports.item-analysis', $exam->id) }}" class="btn btn-secondary btn-sm">
                🔍 Analisis Butir Soal
            </a>
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
            ...
            <form method="GET" action="{{ route('admin.reports.exam', $exam->id) }}" style="display: flex; gap: 8px; flex-wrap: wrap;">
                <input 
                    type="text" 
                    name="search" 
                    class="form-control" 
                ...
                @if(request('search') || request('class_id'))
                    <a href="{{ route('admin.reports.exam', $exam->id) }}" class="btn btn-secondary">Reset</a>
                @endif
            </form>
        </div>
        ...
        @else
            <div class="empty-state">
                ...
                <div class="empty-desc">Data nilai akan tampil di sini setelah peserta menyelesaikan pengerjaan ujian.</div>
            </div>
        @endif
    </div>
