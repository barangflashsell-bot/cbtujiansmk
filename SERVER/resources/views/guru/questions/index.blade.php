...
<div class="content-header">
    <div>
        ...
        <p class="page-subtitle">Kelola butir soal ujian yang Anda susun beserta pilihan dan kunci jawaban</p>
    </div>
    <div style="display: flex; gap: 8px;">
        <button type="button" class="btn btn-secondary" onclick="openImportModal()">
            <span>📊</span> Import Soal Excel
        </button>
        <a href="{{ route('guru.questions.create') }}" class="btn btn-primary">+ Buat Soal Baru</a>
    </div>
</div>

<div class="card">
    <form method="GET" action="{{ route('guru.questions.index') }}" class="search-filter-bar">
        ...
        @if(request('search') || request('subject_id') || request('type'))
            <a href="{{ route('guru.questions.index') }}" class="btn btn-secondary">Reset</a>
        @endif
    </form>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="empty-state">
                            ...
                            <div class="empty-desc">Klik tombol "Buat Soal Baru" di atas untuk mulai membuat bank soal.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
