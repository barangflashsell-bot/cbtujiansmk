...
@section('title', 'Bank Soal Saya - CBT Guru')

@section('content')
<div class="content-header">
    <div>
        <h1 class="page-title">Bank Soal Saya</h1>
        <p class="page-subtitle">Kelola butir soal ujian per-mata pelajaran yang Anda susun beserta pilihan dan kunci jawaban</p>
    </div>
    <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
        ...
        <a href="{{ route('guru.questions.create') }}" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 6px;">
...
<div class="card" style="margin-bottom: 24px;">
    <div style="padding: 16px 20px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div>
                <h3 style="margin: 0; font-size: 15px; font-weight: 700; color: var(--text-primary);">Daftar Mata Pelajaran &amp; Bank Soal Anda</h3>
                <span style="font-size: 12px; color: var(--text-muted);">Pilih mata pelajaran untuk membuat butir soal atau kelola soal yang Anda miliki</span>
            </div>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
            <form method="GET" action="{{ route('guru.questions.index') }}" style="display: flex; gap: 6px; align-items: center;">
                ...
                @if(request('search_subject'))
                    <a href="{{ route('guru.questions.index') }}" class="btn btn-sm btn-secondary">Reset</a>
                @endif
            </form>
        </div>
    </div>
                ...
                @forelse($subjects as $idx => $sb)
                    <tr class="{{ $selectedSubjectId == $sb->id ? 'table-row-selected' : '' }}" style="{{ $selectedSubjectId == $sb->id ? 'background-color: rgba(2, 132, 199, 0.06);' : '' }}">
                                ...
                                <div>
                                    ...
                                    <div style="display: flex; gap: 6px; align-items: center; margin-top: 3px;">
                                        ...
                                        <span class="badge {{ $sb->questions_count > 0 ? 'badge-info' : 'badge-light' }}" style="font-size: 11px;">
                                            {{ $sb->questions_count }} Butir Soal Anda
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <td style="text-align: center;">
                            <a href="{{ route('guru.questions.create', ['subject_id' => $sb->id]) }}" class="btn btn-sm btn-primary" style="display: inline-flex; align-items: center; gap: 6px; font-weight: 600; padding: 6px 14px;">
                        ...
                        <td style="text-align: center;">
                            <div style="display: inline-flex; gap: 6px; align-items: center; justify-content: center;">
                                @if($selectedSubjectId == $sb->id)
                                    <a href="{{ route('guru.questions.index') }}" class="btn btn-sm btn-secondary" title="Sembunyikan daftar butir soal">
                                        <span>✕</span> Tutup Soal
                                    </a>
                                @else
                                    <a href="{{ route('guru.questions.index', ['subject_id' => $sb->id]) }}#detail-soal" class="btn btn-sm btn-secondary" style="border-color: #bae6fd; color: #0284c7;" title="Lihat dan kelola butir soal mata pelajaran ini">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="empty-state">
                            ...
                            <div class="empty-desc">Hubungi administrator untuk menambahkan mata pelajaran.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="card" id="detail-soal" style="border: 1px solid #bae6fd; box-shadow: 0 4px 14px rgba(2, 132, 199, 0.08); margin-bottom: 24px;">
        <div style="padding: 16px 20px; border-bottom: 1px solid #e0f2fe; background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    <div style="font-size: 11px; font-weight: 700; text-transform: uppercase; color: #0284c7; letter-spacing: 0.5px;">Repository Butir Soal Anda</div>
                    ...
                    <div style="font-size: 12px; color: #475569; margin-top: 2px;">
                        Total: <strong>{{ $questions->total() }} Butir Soal</strong>
                    </div>
                </div>
            </div>
            <div style="display: flex; gap: 8px; align-items: center;">
                @if($selectedSubject)
                    <a href="{{ route('guru.questions.create', ['subject_id' => $selectedSubject->id]) }}" class="btn btn-sm btn-primary" style="font-weight: 700;">
                        <span>+</span> Buat Soal {{ $selectedSubject->name }}
                    </a>
                @endif
                <a href="{{ route('guru.questions.index') }}" class="btn btn-sm btn-secondary">
        ...
        <div style="padding: 14px 20px; border-bottom: 1px solid var(--border-color);">
            <form method="GET" action="{{ route('guru.questions.index') }}" class="search-filter-bar" style="margin: 0;">
                @if($selectedSubject)
                    <input type="hidden" name="subject_id" value="{{ $selectedSubject->id }}">
                @endif
                <div class="search-input-group">
                    <span class="search-icon">&#128269;</span>
                    <input type="text" name="search" class="form-control" placeholder="Cari butir soal Anda..." value="{{ request('search') }}">
                </div>
                ...
                @if(request('search') || request('type'))
                    <a href="{{ route('guru.questions.index', $selectedSubject ? ['subject_id' => $selectedSubject->id] : []) }}" class="btn btn-secondary">Reset</a>
                @endif
            </form>
        </div>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <div style="display: inline-flex; gap: 6px;">
                                    <a href="{{ route('guru.questions.edit', $q->id) }}" class="btn btn-sm btn-secondary">Edit</a>
                                    <form method="POST" action="{{ route('guru.questions.destroy', $q->id) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus butir soal ini?');" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="empty-state">
                                <div class="empty-icon">&#128221;</div>
                                <div class="empty-title">Belum ada butir soal yang Anda susun</div>
                                <div class="empty-desc">
                                    @if($selectedSubject)
                                        Klik tombol "Buat Soal {{ $selectedSubject->name }}" di atas untuk mulai membuat butir soal.
                                    @else
        ...
        <form action="{{ route('guru.questions.import') }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="margin-bottom: 16px;">
                <p style="font-size: 12.5px; color: var(--text-secondary); margin-bottom: 12px; line-height: 1.45;">
                    Unggah butir soal dalam format <strong>Excel (.xlsx)</strong> atau <strong>CSV (.csv)</strong>.
                </p>
                ...
                <div style="background: var(--bg-surface-elevated); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px 14px; margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; gap: 10px;">
                    <div>
                        <div style="font-size: 12.5px; font-weight: 700; color: var(--text-primary);">Template Standar Excel</div>
                        <div style="font-size: 11px; color: var(--text-muted);">Unduh format tabel soal CBT</div>
                    </div>
                    <a href="{{ route('guru.questions.template') }}" class="btn btn-secondary btn-sm" style="color: #0284c7; border-color: #bae6fd; text-decoration: none;">
