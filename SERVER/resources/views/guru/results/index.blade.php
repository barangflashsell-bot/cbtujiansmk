...
@section('title', 'Rekap Hasil Ujian - CBT Guru')

@section('content')
<div class="content-header">
    <div>
        <h1 class="page-title">Hasil & Nilai Ujian</h1>
        <p class="page-subtitle">Rekapitulasi skor penilaian dan status kelulusan peserta pada paket ujian Anda</p>
    </div>
</div>

<div class="card">
    <form method="GET" action="{{ route('guru.results.index') }}" class="search-filter-bar">
        ...
        <div class="filter-select-group">
            <select name="exam_id" class="form-select" onchange="this.form.submit()">
                <option value="">Semua Paket Ujian Saya</option>
                @foreach($exams as $ex)
        ...
        @if(request('search') || request('exam_id') || request('class_id') || request('is_published') !== null)
            <a href="{{ route('guru.results.index') }}" class="btn btn-secondary">Reset</a>
        @endif
    </form>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <div style="display: inline-flex; gap: 4px;">
                                <a href="{{ route('guru.results.show', $r->id) }}" class="btn btn-sm btn-secondary">Detail</a>
                                <form method="POST" action="{{ route('guru.results.publish', $r->id) }}" style="display:inline;">
                                    @csrf
                ...
                @empty
                    <tr>
                        <td colspan="8" class="empty-state">
                            ...
                            <div class="empty-desc">Hasil pengerjaan siswa pada paket ujian Anda akan tercatat di sini.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
