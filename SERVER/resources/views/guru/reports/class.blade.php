...
@section('title', 'Laporan Rekapitulasi Kelas: ' . $class->name . ' — CBT Guru')
@section('page-title', 'Laporan Rekapitulasi Kelas Guru')
...
<div style="display: flex; flex-direction: column; gap: 24px;">
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
            <h3 class="card-title" style="margin-bottom: 0;">Rekapitulasi Skor Siswa (Ujian Anda)</h3>

            <form method="GET" action="{{ route('guru.reports.class', $class->id) }}" style="display: flex; gap: 8px; flex-wrap: wrap;">
                <input 
                    type="text" 
                    name="search" 
                    class="form-control" 
                ...
                <select name="exam_id" class="form-select" onchange="this.form.submit()" style="max-width: 220px;">
                    <option value="">Semua Paket Ujian Anda</option>
                    @foreach($exams as $e)
                ...
                @if(request('search') || request('exam_id'))
                    <a href="{{ route('guru.reports.class', $class->id) }}" class="btn btn-secondary">Reset</a>
                @endif
            </form>
        </div>
        ...
        @else
            <div class="empty-state">
                ...
                <div class="empty-desc">Siswa pada rombel ini belum memiliki riwayat nilai dari paket ujian Anda.</div>
            </div>
        @endif
    </div>
